...
describe QueryParams do
  ...
  subject { QueryParams.new(base_uri) }
  ...
  describe '#build_uri' do
    ...
    it 'from hash' do
      uri = QueryParams.build_uri(base_uri: base_uri, conditions: ["name  =  James Rodrigues", "age >= 10"])
    ...
    it 'should encode URL' do
      uri = QueryParams.build_uri(conditions: "desc = Programming Ruby: The Pragmatic Programmer's Guide")
    ...
    it 'invalid operator' do
      expect{ QueryParams.build_uri(conditions: "age <> 13") }.to raise_error(ArgumentError)
    end
    ...
    it 'full_text_search on hash' do
      expect(QueryParams.build_uri(q: "James")).to eq "q=James"
    end
  end
end